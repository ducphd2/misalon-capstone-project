import { IQuery } from '@libs/interfaces';
...
export class QueryUtils {
  ...
  async buildQuery(orderBy: string, first: number, last: number, before: string, after: string): Promise<IQuery> {
    return merge({}, await this.getOrder(orderBy), await this.getCursor(first, last, before, after));
  }
}
